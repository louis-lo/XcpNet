...
namespace XcpNet.Supplier.Controllers
{
    public sealed class ProductManager : SupplierController
    {
        ...
        public void Manager(string state, int page = 1)
        {
            ...
            {
                where = (new DbWhere<P.Product>("State", P.ProductState.Sale) | new DbWhere<P.Product>("State", P.ProductState.BeforeSaved));
                where &= new DbWhere<P.Product>("SupplierId").InSelect<P.Supplier>(new DbSelect("UserId")).Where(new DbWhere("Subjection", 0) & new DbWhere("SupplierType", 0)).Result();
            ...
            if (state == "whole")///全国优选
            {
                ...
                list = Db<P.Product>.Query(DataSource)
                 .Select(new DbSelect<P.Product>(), new DbSelect<M.Member>("Name"), new DbSelect<P.ProductCategory>("Name"), new DbSelect<P.ProductAreaMapping>("Saled"))
                 .InnerJoin(new DbColumn<P.Product>("SupplierId"), new DbColumn<M.Member>("Id"))
            ...
            {
                list = Db<P.Product>.Query(DataSource)
                 .Select(new DbSelect<P.Product>(), new DbSelect<M.Member>("Name"), new DbSelect<P.ProductCategory>("Name"))
                 .InnerJoin(new DbColumn<P.Product>("SupplierId"), new DbColumn<M.Member>("Id"))
                 .InnerJoin(new DbColumn<P.Product>("CategoryId"), new DbColumn<P.ProductCategory>("Id"))
                 .Where(where)
                 .OrderBy(new DbOrderBy<P.Product>("CreationDate", DbOrderByType.Desc))
