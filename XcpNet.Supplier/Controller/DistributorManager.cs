...
namespace XcpNet.Supplier.Controllers
{
    public sealed class DistributorManager : SupplierController
    {
        ...
        public void Manager(string state, int page = 1)
        {
            ...
            {
                where = (new DbWhere<D.DistributorProduct>("State", P.ProductState.Sale) | new DbWhere<D.DistributorProduct>("State", P.ProductState.BeforeSaved));
                where &= new DbWhere<D.DistributorProduct>("SupplierType", 0);
            ...
            if (state == "whole")///全国优选
            {
                ...
                list = Db<D.DistributorProduct>.Query(DataSource)
                 .Select(new DbSelect<D.DistributorProduct>(), new DbSelect<Cnaws.Product.Modules.Supplier>("Company"), new DbSelect<D.DistributorCategory>("Name"), new DbSelect<D.DistributorAreaMapping>("Saled"))
                 .InnerJoin(new DbColumn<D.DistributorProduct>("SupplierId"), new DbColumn<Cnaws.Product.Modules.Supplier>("UserId"))
            ...
            {
                list = Db<D.DistributorProduct>.Query(DataSource)
                 .Select(new DbSelect<D.DistributorProduct>(), new DbSelect<Cnaws.Product.Modules.Supplier>("Company"), new DbSelect<D.DistributorCategory>("Name"))
                 .InnerJoin(new DbColumn<D.DistributorProduct>("SupplierId"), new DbColumn<Cnaws.Product.Modules.Supplier>("UserId"))
                 .InnerJoin(new DbColumn<D.DistributorProduct>("CategoryId"), new DbColumn<D.DistributorCategory>("Id"))
                 .Where(where)
                 .OrderBy(new DbOrderBy<D.DistributorProduct>("CreationDate", DbOrderByType.Desc))
