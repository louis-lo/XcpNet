...
namespace XcpNet.Supplier.Controllers.Extension
{
    public class Freight : SupplierController
    {
        ...
        private void Template(long user, long templateId = 0)
        {
            ...
            if (IsSupplier())
                ...
                Render("freight_template.html");
        }
        [SupplierOrDistributor(true)]
        public void Template(long templateId = 0)
        {
            Template(User.Identity.Id, templateId);
        }
        [AdminAuthorize]
        ...
        public void Submit(int id = 0)
        {
            ...
            try
            {
                ...
                {
                    FreightTemplate.Number = (int)(double.Parse(Request.Form["Number"]) * 100);
                    FreightTemplate.StepNumber = (int)(double.Parse(Request.Form["StepNumber"]) * 100);
                }
                ...
                foreach (string key in Keys)
                {
                    ...
                    if (MappingId != 0)
                    {
                        ...
                        if (FreightTemplate.Type != P.FreightTemplate.ValuationType.ThePrece)
                        {
                            FreightMapaing.Number = (int)(double.Parse(Request.Form["Number_" + MappingId]) * 100);
                            FreightMapaing.StepNumber = (int)(double.Parse(Request.Form["StepNumber_" + MappingId]) * 100);
                        }
                        else
                        {
