...
{
    public class Order : SupplierController
    {
        ...
        public void List(string type, long page = 0L)
        {
            ...
            switch (type)
            {
                ...
                case "payment":                    
                    this["OrderList"] = P.ProductOrder.GetPageBySupplier(DataSource, User.Identity.Id, (int)P.OrderState.Payment, search.title,search.nickName,search.startDate,search.endDate, Math.Max(1, page), 10, 8);
                    break;
                case "delivery":

                    this["OrderList"] = P.ProductOrder.GetPageBySupplier(DataSource, User.Identity.Id, (int)P.OrderState.Delivery, search.title, search.nickName, search.startDate, search.endDate, Math.Max(1, page), 10, 8);
                    break;
                ...
                case "all":
                    if (page < 1L) page = 1L;
                    this["OrderList"] = P.ProductOrder.GetPageBySupplier(DataSource, User.Identity.Id, -1, search.title, search.nickName, search.startDate, search.endDate, page, 10, 8);
                    break;
                case "finished":
                    if (page < 1L) page = 1L;
                    this["OrderList"] = P.ProductOrder.GetPageBySupplier(DataSource, User.Identity.Id, (int)P.OrderState.Finished, search.title, search.nickName, search.startDate, search.endDate, page, 10, 8);
                    break;
                case "getlogistics":
            ...
            this["State"] = type;
            if (IsSupplier())
                Render(string.Concat("supplier/order_", type, ".html"));
