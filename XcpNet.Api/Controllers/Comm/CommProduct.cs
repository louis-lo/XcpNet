...
{
    public class CommProduct : CommonControllers
    {
        public static string ClassName = "[type]Product";
        ...
        public void GetByCategory()
        {
            ...
            if (CheckMark(out mark))
            {
                ...
                if (int.TryParse(Request["Id"], out id))
                {
                    ...
                    SetResult(Pd.Product.ApiGetPageByCategory(DataSource, id, cates.Count,filter,8,1));
                }
                else
                {
                    SetResult(false);
        ...
        {
            CheckMarkHelper(ClassName, "GetByCategory", "根据分类获取产品")
        ...
        {
            CheckMarkHelper(ClassName, "GetCategory", "根据父分类Id获取分类")
                .AddArgument("Id", typeof(int), "父分类Id,默认为0")
        ...
        public void SearchByKeyWord()
        {
            ...
            if (CheckMark(out mark))
            {
                try
                {
                    ...
                    SetResult(Pd.Product.ApiGetPageBySearch(DataSource, keyword, filter, page, size, 8));
                }
                catch (Exception)
                {
                    SetResult(false);
        ...
        {
            CheckMarkHelper(ClassName, "SearchByKeyWord", "根据关键词获取产品")
        ...
        {
            CheckMarkHelper(ClassName, "GetHotKeyWord", "获取热门搜索关键词")
        ...
        public void GetMonthHotPage()
        {
            ...
            if (CheckMark(out mark))
            {
                ...
                if (!int.TryParse(Request["page"], out page) || page < 1)
                    page = 1;
                SetResult(Pd.Product.GetMonthHotPageApi(DataSource, page, size, -1, 8));
            }
        }
        ...
        {
            CheckMarkHelper(ClassName, "GetMonthHotPage", "获取为你推荐产品")
                 .AddArgument("page", typeof(int), "当前页,默认为1")
        ...
        {
            CheckMarkHelper(ClassName, "GetTopArticle", "获取网站公告")
                 .AddArgument("size", typeof(int), "显示条数,默认为10")
