...
{
    public class ApiComment : CommonControllers
    {
        /// <summary>
        /// 获取产品评论的列表
        /// </summary>
        public void GetList()
        {
            ...
            {
                int type = 1, state = 0, star1 = 0, star2 = 0;
                long id, page = 1;
                ...
                {
                    case 1:
                        data = C.Comment.GetPageByTypeAndIdAndStar(DataSource, type, id, star1, star2, Math.Max(page, 1), 20, 11);
                        break;
                    case 2:
                        data = C.Comment.GetPageByTypeAndIdAndImage(DataSource, type, id, Math.Max(page, 1), 20, 11);
                        break;
                    default:
                        data = C.Comment.GetPageByTypeAndId(DataSource, type, id, Math.Max(page, 1), 20, 11);
                        break;
                }
                SetResult(data);
            }
        ...
        {
            CheckMarkHelper("ApiComment", "GetList", "获取产品评论列表")
        ...
        {
            CheckMemberHelper("ApiComment", "GetUnComment", "获取未评论列表")
            ...
            if (CheckMember(out member))
            {
                ...
                try
                {
                    ...
                    if (Db<P.ProductOrderMapping>.Query(DataSource)
                        .Update()
                        .Set("Evaluation", true)
                        .Where(new DbWhere("OrderId", comment.TargetData) & new DbWhere("ProductId", comment.TargetId))
                        .Execute() != 1)
                    {
        ...
        {
            CheckMarkHelper("ApiComment", "SetComment", "设置产品的评论")
