...
{
    public class AfterSales2 : CommControllers2
    {
        public static string ClassName = "[type]AfterSales2";
        ...
        public void SetAfterSales()
        {
            ...
            if (CheckMember(out member))
            {
                try
                {
                    ...
                    code = CommAfterSales.SetAfterSales<Pd.ProductOrder>(DataSource, member, Request.Form["orderid"], long.Parse(Request.Form["productid"]), Request.Form["servicetype"], Money.Parse(Request.Form["refundmoney"]), Request.Form["refundcount"], Request.Form["isfreightamount"], Request["reason"], image, message, out data);
