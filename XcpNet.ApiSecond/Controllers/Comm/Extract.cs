...
{
    public class Extract2 : CommControllers2
    {
        public static string ClassName = "[type]Extract2";
        ...
        {
            CheckMarkApi(ClassName, "GetBank", "获取银行列表")
        ...
        {
            CheckMemberApi(ClassName, "GetUserBank", "获取用户绑定的银行列表")
        ...
        public void DelUserBank()
        {
            try
            {
                ...
                if (CheckMember(out member))
                {
                    int code = CommUtility.PROGRAM_ERROR;
                    try
                    {
                        ...
                        if (!M.MemberInfo.ApiCheckPayPassword(DataSource, member.Id, paypassword))
                        {
                            code = CommUtility.PASSWORD_EQUALS;
                            throw new AggregateException();
                        }
                        if (Db<M.MemberBank>.Query(DataSource).Delete().Where(new DbWhere("Id", id) & new DbWhere("UserId", member.Id)).Execute() > 0)
                            ...
                            SetResult(CommUtility.DEL_FAIL);
                    }
        ...
        {
            CheckMemberApi(ClassName, "DelUserBank", "删除绑定的银行卡")
                .AddArgument("Id", typeof(int), "银行Id")
                .AddArgument("PayPassword", typeof(string), "支付密码（加密过后的）")
                .AddResult(CommUtility.PASSWORD_EQUALS, "支付密码错误")
        ...
        {
            CheckMemberApi(ClassName, "BindCard", "绑定银行卡")
            ...
            if (CheckMember(out member))
            {
                ...
                else if (DrawType == 1)
                {
                    ...
                    try
                    {
                        ...
                        {
                            SetResult(CommUtility.BANK_EMPTY);
                            throw new AggregateException();
                        }
        ...
        {
            CheckMemberApi(ClassName, "ApplyExtract", "申请提现")
                ...
                .AddResult(CommUtility.PRODUCT_ERROR, "参数错误")
                .AddResult(CommUtility.INSERT_FAIL, "插入提现记录失败")
                .AddResult(CommUtility.PROGRAM_ERROR, "程序错误")
                .AddResult(true, typeof(DataStatus), "返回结果");
        }
#endif
        ...
        {
            CheckMemberApi(ClassName, "GetWithDrawList", "获取提现列表")
